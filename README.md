# Fleet (osquery) — endpoint inventory & queries


**What it is:** FleetDM server orchestrating osquery on endpoints; schedule queries, policies, and packs.  
**Replaces/reduces:** Commercial endpoint analytics (e.g., parts of Tanium), inventory scanners.  
**Integrates with:** g-oss-edr-siem (send results to Wazuh/OpenSearch), g-oss-observability.


    ## Components
    - See `docker-compose.yml` for images and versions.
    - Helm chart skeleton is under `charts/g-oss-fleet-osquery` (namespace defaults to `platform`).
    ...
    ```
    Endpoints ↔ osquery agent ↔ FleetDM (API/GUI) → log pipeline (OpenSearch/Loki)
    ```
    ...
    - For Helm, set values in `charts/g-oss-fleet-osquery/values.yaml`.
